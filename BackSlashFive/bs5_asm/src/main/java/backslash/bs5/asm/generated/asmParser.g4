...
statement: 
    label? instruction? Bs5comment? Bs5nl
      { prg.addSourceLine($text).incrementLine(); }
    ;

label:
    Bs5_identifier COLON
      { prg.addLabel($Bs5_identifier.text); }
    ;
...
origin
    : Bs5_org number16bitsNumeral
      { prg.org($number16bitsNumeral.immediat);}
    ;

dataword
    : Bs5_dw wordlist
      { prg.dw($wordlist.lstWord); }
    ;
...
string
    ...
    : DBLQUOTE STRDATA DBLQUOTEEND
      { $lstWord = new ArrayList<String>(); for (char i: $STRDATA.text.toCharArray()) $lstWord.add(""+((int)i));}
    ;
...
bs5_core_instruction 
    ...
    : bs5_cond? bs5_flag? 
      Bs5_mov bs5_reg COMMA numberZero
        { prg.asm_mov_Rx_0($bs5_cond.text, $bs5_flag.text, $bs5_reg.text); }

    // ccc f mov low Rx, 0
    | bs5_cond? bs5_flag? 
      Bs5_mov Bs5_low bs5_reg COMMA numberZero
        { prg.asm_mov_low_Rx_0($bs5_cond.text, $bs5_flag.text, $bs5_reg.text); } 

    // ccc f mov high Rx, 0
    | bs5_cond? bs5_flag? 
      Bs5_mov Bs5_high bs5_reg COMMA numberZero
        { prg.asm_mov_high_Rx_0($bs5_cond.text, $bs5_flag.text, $bs5_reg.text); }  

    // ccc f mov Rx, Ry
    | bs5_cond? bs5_flag? 
      Bs5_mov rx=bs5_reg COMMA ry=bs5_reg
        { prg.asm_mov_Rx_Ry($bs5_cond.text, $bs5_flag.text, $rx.text, $ry.text); }

    // ccc f mov Rx, [Ry]
    | bs5_cond? bs5_flag? 
      Bs5_mov rx=bs5_reg COMMA OPEN_BRACKET ry=bs5_reg CLOSE_BRACKET
        { prg.asm_mov_Rx_atRy($bs5_cond.text, $bs5_flag.text, $rx.text, $ry.text); }

    // ccc f mov [Rx], Ry
    | bs5_cond? bs5_flag? 
      Bs5_mov OPEN_BRACKET rx=bs5_reg CLOSE_BRACKET COMMA ry=bs5_reg
        { prg.asm_mov_atRx_Ry($bs5_cond.text, $bs5_flag.text, $rx.text, $ry.text); }

    // ccc f mov low R0, imm8 (imm8 != 0)
    | bs5_cond? bs5_flag? 
      Bs5_mov Bs5_low Bs5_reg0 COMMA numberUnsignedByteNotZero
        { prg.asm_mov_low_R0_imm8($bs5_cond.text, $bs5_flag.text, $numberUnsignedByteNotZero.immediat); }

    // ccc f mov high R0, imm8 (imm8 != 0)
    | bs5_cond? bs5_flag? 
      Bs5_mov Bs5_high Bs5_reg0 COMMA numberUnsignedByteNotZero
        { prg.asm_mov_high_R0_imm8($bs5_cond.text, $bs5_flag.text, $numberUnsignedByteNotZero.immediat); }

    // ccc f mov low R0, low Rx
    | bs5_cond? bs5_flag? 
      Bs5_mov Bs5_low Bs5_reg0 COMMA Bs5_low bs5_reg
        { prg.asm_mov_low_R0_low_Rx($bs5_cond.text, $bs5_flag.text, $bs5_reg.text); }

    // ccc f mov high R0, low Rx
    | bs5_cond? bs5_flag? 
      Bs5_mov Bs5_high Bs5_reg0 COMMA Bs5_low bs5_reg
        { prg.asm_mov_high_R0_low_Rx($bs5_cond.text, $bs5_flag.text, $bs5_reg.text); }

    // ccc f mov low R0, high Rx
    | bs5_cond? bs5_flag? 
      Bs5_mov Bs5_low Bs5_reg0 COMMA Bs5_high bs5_reg
        { prg.asm_mov_low_R0_high_Rx($bs5_cond.text, $bs5_flag.text, $bs5_reg.text); }

    // ccc f mov high R0, high Rx
    | bs5_cond? bs5_flag? 
      Bs5_mov Bs5_high Bs5_reg0 COMMA Bs5_high bs5_reg
        { prg.asm_mov_high_R0_high_Rx($bs5_cond.text, $bs5_flag.text, $bs5_reg.text); }

    // ccc f mov X, Rx:imm4
    | bs5_cond? bs5_flag? 
      Bs5_mov Bs5_flag_x COMMA bs5_reg COLON numberUnsignedQuad
        { prg.asm_mov_X_Rx_imm4($bs5_cond.text, $bs5_flag.text, $bs5_reg.text, $numberUnsignedQuad.immediat); }

    // ccc f mov Rx:imm4, X
    | bs5_cond? bs5_flag? 
      Bs5_mov bs5_reg COLON numberUnsignedQuad COMMA Bs5_flag_x
        { prg.asm_mov_Rx_imm4_X($bs5_cond.text, $bs5_flag.text, $bs5_reg.text, $numberUnsignedQuad.immediat); }

    // ccc f mov Rx:imm4, 0
    | bs5_cond? bs5_flag? 
      Bs5_mov bs5_reg COLON numberUnsignedQuad COMMA numberZero
        { prg.asm_mov_Rx_imm4_0($bs5_cond.text, $bs5_flag.text, $bs5_reg.text, $numberUnsignedQuad.immediat); }

    // ccc f mov Rx:imm4, 1    
    | bs5_cond? bs5_flag?
      Bs5_mov bs5_reg COLON numberUnsignedQuad COMMA numberOne
        { prg.asm_mov_Rx_imm4_1($bs5_cond.text, $bs5_flag.text, $bs5_reg.text, $numberUnsignedQuad.immediat); }

    // ccc f add Rx, Ry
    | bs5_cond? bs5_flag? 
      Bs5_add rx=bs5_reg COMMA ry=bs5_reg
        { prg.asm_add_Rx_Ry($bs5_cond.text, $bs5_flag.text, $rx.text, $ry.text); }

    // ccc f add Rx, [Ry]
    | bs5_cond? bs5_flag? 
      Bs5_add rx=bs5_reg COMMA OPEN_BRACKET ry=bs5_reg CLOSE_BRACKET
        { prg.asm_add_Rx_atRy($bs5_cond.text, $bs5_flag.text, $rx.text, $ry.text); }

    // ccc f add R0, imm4
    | bs5_cond? bs5_flag? 
      Bs5_add Bs5_reg0 COMMA numberUnsignedQuad
        { prg.asm_add_R0_imm4($bs5_cond.text, $bs5_flag.text, $numberUnsignedQuad.immediat); }

    // ccc f add Rx, 1  (Rx != R0 preferably)
    | bs5_cond? bs5_flag? 
      Bs5_add bs5_reg_1_15 COMMA numberOne
        { prg.asm_add_Rx_1($bs5_cond.text, $bs5_flag.text, $bs5_reg_1_15.text);}

    // ccc f add R15, simm8
    | bs5_cond? bs5_flag? 
      Bs5_add Bs5_reg15 COMMA numberSignedByte
        { prg.asm_add_R15_simm8($bs5_cond.text, $bs5_flag.text, $numberSignedByte.immediat);  }

    // ccc f sub Rx, Ry
    | bs5_cond? bs5_flag? 
      Bs5_sub rx=bs5_reg COMMA ry=bs5_reg
        { prg.asm_add_Rx_Ry($bs5_cond.text, $bs5_flag.text, $rx.text, $ry.text); }

    // ccc f sub Rx, [Ry]
    | bs5_cond? bs5_flag? 
      Bs5_sub rx=bs5_reg COMMA OPEN_BRACKET ry=bs5_reg CLOSE_BRACKET
        { prg.asm_sub_Rx_atRy($bs5_cond.text, $bs5_flag.text, $rx.text, $ry.text); }

    // ccc f sub R0, imm4
    | bs5_cond? bs5_flag? 
      Bs5_sub Bs5_reg0 COMMA numberUnsignedQuad
        { prg.asm_sub_R0_imm4($bs5_cond.text, $bs5_flag.text, $numberUnsignedQuad.immediat); }

    // ccc f sub Rx, 1  (Rx != R0 preferably)
    | bs5_cond? bs5_flag? 
      Bs5_sub bs5_reg_1_15 COMMA numberOne
        { prg.asm_sub_Rx_1($bs5_cond.text, $bs5_flag.text, $bs5_reg_1_15.text);}

    // ccc f shl R0, imm4
    | bs5_cond? bs5_flag? 
      Bs5_shl Bs5_reg0 COMMA numberUnsignedQuad
        { prg.asm_shl_R0_imm4($bs5_cond.text, $bs5_flag.text, $numberUnsignedQuad.immediat);}

    // ccc f shr R0, imm4
    | bs5_cond? bs5_flag? 
      Bs5_shr Bs5_reg0 COMMA numberUnsignedQuad
        { prg.asm_shr_R0_imm4($bs5_cond.text, $bs5_flag.text, $numberUnsignedQuad.immediat);}

    // ccc f and R0, Rx
    | bs5_cond? bs5_flag? 
      Bs5_and Bs5_reg0 COMMA bs5_reg
        { prg.asm_and_R0_Rx($bs5_cond.text, $bs5_flag.text, $bs5_reg.text);}

    // ccc f or R0, Rx
    | bs5_cond? bs5_flag? 
      Bs5_or Bs5_reg0 COMMA bs5_reg
        { prg.asm_or_R0_Rx($bs5_cond.text, $bs5_flag.text, $bs5_reg.text);}

    // ccc f not Rx:imm4
    | bs5_cond? bs5_flag? 
      Bs5_not bs5_reg COLON numberUnsignedQuad
        { prg.asm_not_Rx_imm4($bs5_cond.text, $bs5_flag.text, $bs5_reg.text, $numberUnsignedQuad.immediat);}

    // ccc f not Rx    
    | bs5_cond? bs5_flag? 
      Bs5_not bs5_reg
        { prg.asm_not_Rx($bs5_cond.text, $bs5_flag.text, $bs5_reg.text);}
    ;
...
bs5_corex_instruction 
    ...
    : bs5_cond? bs5_flag? 
      Bs5_mov Bs5_low bs5_reg_1_15 COMMA numberUnsignedByte
        { prg.asm_mov_low_Rx_imm8($bs5_cond.text, $bs5_flag.text, $bs5_reg_1_15.text, $numberUnsignedByte.immediat);}

    // ccc f  mov high Rx, imm8
    | bs5_cond? bs5_flag? 
      Bs5_mov Bs5_high bs5_reg_1_15 COMMA numberUnsignedByte
        { prg.asm_mov_high_Rx_imm8($bs5_cond.text, $bs5_flag.text, $bs5_reg_1_15.text, $numberUnsignedByte.immediat);}

    // ccc f add Rx, imm4 (Rx != R0)
    | bs5_cond? bs5_flag? 
      Bs5_add bs5_reg_1_15 COMMA numberUnsignedQuadNotOne
        { prg.asm_add_Rx_imm4($bs5_cond.text, $bs5_flag.text, $bs5_reg_1_15.text, $numberUnsignedQuadNotOne.immediat ); }

    // ccc f sub Rx, imm4 (Rx != R0)
    | bs5_cond? bs5_flag? 
      Bs5_sub bs5_reg_1_15 COMMA numberUnsignedQuadNotOne
        { prg.asm_sub_Rx_imm4($bs5_cond.text, $bs5_flag.text, $bs5_reg_1_15.text, $numberUnsignedQuadNotOne.immediat ); }

    // ccc f shl Rx, imm4 (Rx != R0)
    | bs5_cond? bs5_flag? 
      Bs5_shl bs5_reg_1_15 COMMA numberUnsignedQuad
        { prg.asm_shl_Rx_imm4($bs5_cond.text, $bs5_flag.text, $bs5_reg_1_15.text, $numberUnsignedQuad.immediat ); }

    // ccc f shr Rx, imm4 (Rx != R0)
    | bs5_cond? bs5_flag? 
      Bs5_shr bs5_reg_1_15 COMMA numberUnsignedQuad
        { prg.asm_shr_Rx_imm4($bs5_cond.text, $bs5_flag.text, $bs5_reg_1_15.text, $numberUnsignedQuad.immediat ); }

    // ccc f mov low Rx, low Ry (Rx != R0 and Ry != R0)
    | bs5_cond? bs5_flag? 
      Bs5_mov Bs5_low rx=bs5_reg_1_15 COMMA Bs5_low ry=bs5_reg_1_15
        { prg.asm_mov_low_Rx_low_Ry($bs5_cond.text, $bs5_flag.text, $rx.text, $ry.text ); }

    // ccc f mov low Rx, high Ry (Rx != R0 and Ry != R0)
    | bs5_cond? bs5_flag? 
      Bs5_mov Bs5_low rx=bs5_reg_1_15 COMMA Bs5_high ry=bs5_reg_1_15
        { prg.asm_mov_low_Rx_high_Ry($bs5_cond.text, $bs5_flag.text, $rx.text, $ry.text ); }

    // ccc f mov high Rx, low Ry (Rx != R0 and Ry != R0)
    | bs5_cond? bs5_flag? 
      Bs5_mov Bs5_high rx=bs5_reg_1_15 COMMA Bs5_low ry=bs5_reg_1_15
        { prg.asm_mov_high_Rx_low_Ry($bs5_cond.text, $bs5_flag.text, $rx.text, $ry.text ); }

    // ccc f mov high Rx, high Ry (Rx != R0 and Ry != R0)
    | bs5_cond? bs5_flag? 
      Bs5_mov Bs5_high rx=bs5_reg_1_15 COMMA Bs5_high ry=bs5_reg_1_15
        { prg.asm_mov_high_Rx_high_Ry($bs5_cond.text, $bs5_flag.text, $rx.text, $ry.text ); }

    // ccc f and Rx, Ry (Rx != R0 and Ry != R0)
    | bs5_cond? bs5_flag? 
      Bs5_and rx=bs5_reg_1_15 COMMA ry=bs5_reg_1_15
        { prg.asm_and_Rx_Ry($bs5_cond.text, $bs5_flag.text, $rx.text, $ry.text ); }

    // ccc f or  Rx, Ry (Rx != R0 and Ry != R0)
    | bs5_cond? bs5_flag? 
      Bs5_or rx=bs5_reg_1_15 COMMA ry=bs5_reg_1_15
      { prg.asm_or_Rx_Ry($bs5_cond.text, $bs5_flag.text, $rx.text, $ry.text ); }
    ;
...
bs5_imm16_instruction
    ...
    : bs5_cond? bs5_flag? 
      Bs5_mov bs5_reg_1_15 COMMA number16bitsNotZero
        { prg.asm_mov_Rx_imm16($bs5_cond.text, $bs5_flag.text, $bs5_reg_1_15.text, $number16bitsNotZero.immediat ); }

    // ccc f mov R0, imm16 ( imm16 != 0 )
    | bs5_cond? bs5_flag? 
      Bs5_mov Bs5_reg0 COMMA number16bitsNotZero
        { prg.asm_mov_R0_imm16($bs5_cond.text, $bs5_flag.text, $number16bitsNotZero.immediat ); }

    // ccc f add Rx, imm16 (Rx != R0 and imm16 > 15)
    | bs5_cond? bs5_flag? 
      Bs5_add bs5_reg_1_15 COMMA number16bitsNotQuad
        { prg.asm_add_Rx_imm16($bs5_cond.text, $bs5_flag.text, $bs5_reg_1_15.text, $number16bitsNotQuad.immediat ); }

    // ccc f sub Rx, imm16 (Rx != R0 and imm16 > 15)
    | bs5_cond? bs5_flag? 
      Bs5_sub bs5_reg_1_15 COMMA number16bitsNotQuad
        { prg.asm_sub_Rx_imm16($bs5_cond.text, $bs5_flag.text, $bs5_reg_1_15.text, $number16bitsNotQuad.immediat ); }

    // ccc f mov Rx, [imm16] 
    | bs5_cond? bs5_flag? 
      Bs5_mov bs5_reg COMMA OPEN_BRACKET number16bits CLOSE_BRACKET
        { prg.asm_mov_Rx_atImm16($bs5_cond.text, $bs5_flag.text, $bs5_reg.text, $number16bits.immediat ); }

    // ccc f mov [imm16], Rx ( Rx != R0 )
    | bs5_cond? bs5_flag? 
      Bs5_mov OPEN_BRACKET number16bits CLOSE_BRACKET COMMA bs5_reg_1_15
        { prg.asm_mov_atImm16_Rx($bs5_cond.text, $bs5_flag.text, $bs5_reg_1_15.text, $number16bits.immediat );}

    // ccc f add Rx, [imm16] ( Rx != R0 )
    | bs5_cond? bs5_flag? 
      Bs5_add bs5_reg_1_15 COMMA OPEN_BRACKET number16bits CLOSE_BRACKET
        { prg.asm_add_Rx_atImm16($bs5_cond.text, $bs5_flag.text, $bs5_reg_1_15.text, $number16bits.immediat ); }

    // ccc f sub Rx, [imm16] ( Rx != R0 )
    | bs5_cond? bs5_flag? 
      Bs5_sub bs5_reg_1_15 COMMA OPEN_BRACKET number16bits CLOSE_BRACKET
        { prg.asm_sub_Rx_atImm16($bs5_cond.text, $bs5_flag.text, $bs5_reg_1_15.text, $number16bits.immediat ); }
    ...
    | bs5_cond? bs5_flag? 
      Bs5_or bs5_reg_1_15 COMMA number16bits
        { prg.asm_or_Rx_imm16($bs5_cond.text, $bs5_flag.text, $bs5_reg_1_15.text, $number16bits.immediat ); }

    // ccc f and Rx, [imm16] (Rx != R0)
    | bs5_cond? bs5_flag? 
      Bs5_and bs5_reg_1_15 COMMA OPEN_BRACKET number16bits CLOSE_BRACKET
        { prg.asm_and_Rx_atImm16($bs5_cond.text, $bs5_flag.text, $bs5_reg_1_15.text, $number16bits.immediat ); }

    // ccc f or Rx, [imm16] (Rx != R0)
    | bs5_cond? bs5_flag? 
      Bs5_or bs5_reg_1_15 COMMA OPEN_BRACKET number16bits CLOSE_BRACKET
        { prg.asm_or_Rx_atImm16($bs5_cond.text, $bs5_flag.text, $bs5_reg_1_15.text, $number16bits.immediat ); }
    ;
...
//   Write to stack/local context (24 microprograms)
    ...
    | bs5_cond? bs5_flag?
      Bs5_mov Bs5_stack COLON numberUnsignedQuad COMMA bs5_reg_1_15
        { prg.asm_mov_stackImm4_Rx($bs5_cond.text, $bs5_flag.text, $numberUnsignedQuad.immediat, $bs5_reg_1_15.text);}

    //ccc f mov STACK:imm4, imm16 ( R0 modified)
    | bs5_cond? bs5_flag?
      Bs5_mov Bs5_stack COLON numberUnsignedQuad COMMA number16bits
        { prg.asm_mov_stackImm4_imm16($bs5_cond.text, $bs5_flag.text, $numberUnsignedQuad.immediat, $number16bits.immediat);}

    //ccc f mov STACK:imm4, [Rx] (Rx != R0 , R0 modified)
    | bs5_cond? bs5_flag?
      Bs5_mov Bs5_stack COLON numberUnsignedQuad COMMA OPEN_BRACKET bs5_reg_1_15 CLOSE_BRACKET
        { prg.asm_mov_stackImm4_atRx($bs5_cond.text, $bs5_flag.text, $numberUnsignedQuad.immediat, $bs5_reg_1_15.text);}

    //ccc f mov STACK:imm4, [imm16] (R0 modified)
    | bs5_cond? bs5_flag?
      Bs5_mov Bs5_stack COLON numberUnsignedQuad COMMA OPEN_BRACKET number16bits CLOSE_BRACKET
        { prg.asm_mov_stackImm4_atImm16($bs5_cond.text, $bs5_flag.text, $numberUnsignedQuad.immediat, $number16bits.immediat);}

    //ccc f mov STACK:imm16, Rx (Rx != R0 , R0 modified)
    | bs5_cond? bs5_flag?
      Bs5_mov Bs5_stack COLON number16bitsNotQuad COMMA bs5_reg_1_15
        { prg.asm_mov_stackImm16_Rx($bs5_cond.text, $bs5_flag.text, $number16bitsNotQuad.immediat, $bs5_reg_1_15.text);}

    //ccc f mov STACK:imm16a, imm16b ( R0 modified)
    | bs5_cond? bs5_flag?
      Bs5_mov Bs5_stack COLON numa1=number16bitsNotQuad COMMA numb1=number16bits
        { prg.asm_mov_stackImm16_imm16($bs5_cond.text, $bs5_flag.text, $numa1.immediat, $numb1.immediat);}

    //ccc f mov STACK:imm16, [Rx] (Rx != R0 , R0 modified)
    | bs5_cond? bs5_flag?
      Bs5_mov Bs5_stack COLON number16bitsNotQuad COMMA OPEN_BRACKET bs5_reg_1_15 CLOSE_BRACKET
        { prg.asm_mov_stackImm16_atRx($bs5_cond.text, $bs5_flag.text, $number16bitsNotQuad.immediat, $bs5_reg_1_15.text);}
    ...
    | bs5_cond? bs5_flag?
      Bs5_mov Bs5_stack COLON rx1=Bs5_reg_1_14 COMMA ry1=Bs5_reg_1_14
        { prg.asm_mov_stackRx_Ry($bs5_cond.text, $bs5_flag.text, $rx1.text, $ry1.text);}

    //ccc f mov STACK:Rx, imm16 (Rx != R0 and R15, R0 modified)
    | bs5_cond? bs5_flag?
      Bs5_mov Bs5_stack COLON Bs5_reg_1_14 COMMA number16bits
        { prg.asm_mov_stackRx_imm16($bs5_cond.text, $bs5_flag.text, $Bs5_reg_1_14.text, $number16bits.immediat);}

    //ccc f mov STACK:Rx, [Ry] (Rx,Ry != R0 , R0 modified)
    | bs5_cond? bs5_flag?
      Bs5_mov Bs5_stack COLON rx2=Bs5_reg_1_14 COMMA OPEN_BRACKET ry2=Bs5_reg_1_14 CLOSE_BRACKET
        { prg.asm_mov_stackRx_atRy($bs5_cond.text, $bs5_flag.text, $rx2.text, $ry2.text);}
    ...
    | bs5_cond? bs5_flag?
      Bs5_mov Bs5_stack COLON Bs5_reg_1_14 COMMA OPEN_BRACKET number16bits CLOSE_BRACKET
        { prg.asm_mov_stackRx_atImm16($bs5_cond.text, $bs5_flag.text, $Bs5_reg_1_14.text, $number16bits.immediat);}

    //ccc f mov LOCAL:imm4, Rx (Rx != R0, R0 modified)
    | bs5_cond? bs5_flag?
      Bs5_mov Bs5_local COLON numberUnsignedQuad COMMA bs5_reg_1_15
        { prg.asm_mov_localImm4_Rx($bs5_cond.text, $bs5_flag.text, $numberUnsignedQuad.immediat, $bs5_reg_1_15.text);}

    //ccc f mov LOCAL:imm4, imm16 ( R0 modified)
      | bs5_cond? bs5_flag?
      Bs5_mov Bs5_local COLON numberUnsignedQuad COMMA number16bits
        { prg.asm_mov_localImm4_imm16($bs5_cond.text, $bs5_flag.text, $numberUnsignedQuad.immediat, $number16bits.immediat);}

    //ccc f mov LOCAL:imm4, [Rx] (Rx != R0 , R0 modified)
      | bs5_cond? bs5_flag?
      Bs5_mov Bs5_local COLON numberUnsignedQuad COMMA OPEN_BRACKET bs5_reg_1_15 CLOSE_BRACKET
        { prg.asm_mov_localImm4_atRx($bs5_cond.text, $bs5_flag.text, $numberUnsignedQuad.immediat, $bs5_reg_1_15.text);}

    //ccc f mov LOCAL:imm4, [imm16] ( R0 modified)
    | bs5_cond? bs5_flag?
      Bs5_mov Bs5_local COLON numberUnsignedQuad COMMA OPEN_BRACKET number16bits CLOSE_BRACKET
        { prg.asm_mov_localImm4_atImm16($bs5_cond.text, $bs5_flag.text, $numberUnsignedQuad.immediat, $number16bits.immediat);}

    //ccc f mov LOCAL:imm16, Rx (Rx != R0 , R0 modified)
    | bs5_cond? bs5_flag?
      Bs5_mov Bs5_local COLON number16bitsNotQuad COMMA bs5_reg_1_15
        { prg.asm_mov_localImm4_Rx($bs5_cond.text, $bs5_flag.text, $number16bitsNotQuad.immediat, $bs5_reg_1_15.text);}

    //ccc f mov LOCAL:imm16a, imm16b ( R0 modified)
    | bs5_cond? bs5_flag?
      Bs5_mov Bs5_local COLON numa3=number16bitsNotQuad COMMA numb3=number16bits
        { prg.asm_mov_localImm16_imm16($bs5_cond.text, $bs5_flag.text, $numa3.immediat, $numb3.immediat);}

    //ccc f mov LOCAL:imm16, [Rx] (Rx != R0 , R0 modified)
    | bs5_cond? bs5_flag?
      Bs5_mov Bs5_local COLON number16bitsNotQuad COMMA OPEN_BRACKET bs5_reg_1_15 CLOSE_BRACKET
        { prg.asm_mov_localImm16_atRx($bs5_cond.text, $bs5_flag.text, $number16bitsNotQuad.immediat, $bs5_reg_1_15.text);}

    //ccc f mov LOCAL:imm16a, [imm16b] ( R0 modified)
    | bs5_cond? bs5_flag?
      Bs5_mov Bs5_local COLON numa4=number16bitsNotQuad COMMA OPEN_BRACKET numb4=number16bits CLOSE_BRACKET
        { prg.asm_mov_localImm16_atImm16($bs5_cond.text, $bs5_flag.text, $numa4.immediat, $numb4.immediat);}

    //ccc f mov LOCAL:Rx, Ry  (Rx and Ry != R15 and R0)
    | bs5_cond? bs5_flag?
      Bs5_mov Bs5_local COLON rx3=Bs5_reg_1_14 COMMA ry3=Bs5_reg_1_14
        { prg.asm_mov_localRx_Ry($bs5_cond.text, $bs5_flag.text, $rx3.text, $ry3.text);}

    //ccc f mov LOCAL:Rx, imm16 (Rx != R0 , R0 modified)
    | bs5_cond? bs5_flag?
      Bs5_mov Bs5_local COLON Bs5_reg_1_14 COMMA number16bits
        { prg.asm_mov_localRx_imm16($bs5_cond.text, $bs5_flag.text, $Bs5_reg_1_14.text, $number16bits.immediat);}
    ...
    | bs5_cond? bs5_flag?
      Bs5_mov Bs5_local COLON rx4=Bs5_reg_1_14 COMMA OPEN_BRACKET ry4=Bs5_reg_1_14 CLOSE_BRACKET
        { prg.asm_mov_localRx_atRy($bs5_cond.text, $bs5_flag.text, $rx4.text, $ry4.text);}

    //ccc f mov LOCAL:Rx, [imm16] (Rx != R0 , R0 modified)
    | bs5_cond? bs5_flag?
      Bs5_mov Bs5_local COLON Bs5_reg_1_14 COMMA OPEN_BRACKET number16bits CLOSE_BRACKET
        { prg.asm_mov_localRx_atImm16($bs5_cond.text, $bs5_flag.text, $Bs5_reg_1_14.text, $number16bits.immediat);}
